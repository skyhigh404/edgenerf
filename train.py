...
def train_step(
  ...
  def loss_fn(variables):
    ...
    renderings = model.apply(
        ...
        compute_extras=(True or config.compute_disp_metrics or
    ...
    for rendering in renderings:
      ...
      depth = rendering['distance_mean'].reshape(-1,4)
      margin = 1e-4
      
      # 计算类别为 1 的平滑损失
      depth_smooth_weight = batch['area_split'].reshape(-1,4)
      depth_1 = depth_smooth_weight * depth
      depth_mean1 = jax.lax.stop_gradient(jnp.nan_to_num(jnp.sum(depth_1, axis=1) / jnp.count_nonzero(depth_smooth_weight,axis=1)))
      depth_loss1 = jnp.mean(jnp.sum(
        jnp.maximum(jnp.abs(depth_1-depth_mean1[:,None])*depth_smooth_weight-margin,0), axis=1))
      
      depth_loss = 0.1 * depth_loss1
      all_depth_loss.append(depth_loss)
      
      
      normals = rendering['normals'].reshape(-1,4,3)
      norm_smooth_weight = batch['area_split'].reshape(-1,4,1)
      normals_1 = norm_smooth_weight * normals
      normals_mean1 = jax.lax.stop_gradient(jnp.nan_to_num(jnp.sum(normals_1, axis=1) / jnp.count_nonzero(norm_smooth_weight,axis=1)))
      
      normal_smooth_loss = jnp.mean(jnp.square(normals-normals_mean1[:,None])*norm_smooth_weight)
      all_normal_loss.append(0.01 * normal_smooth_loss)
    ...
    all_normal_loss = jnp.array(all_normal_loss)
    loss = (
        losses[-1] + config.coarse_loss_mult * jnp.sum(losses[:-1]) + all_depth_loss[-1] + all_normal_loss[-1] +
    ...
    return loss, (losses, disp_mses, normal_maes, weight_l2, losses_georeg, all_depth_loss, all_normal_loss)

  (loss, loss_aux), grad = (jax.value_and_grad(loss_fn, has_aux=True)(
      state.optimizer.target))
  (losses, disp_mses, normal_maes, weight_l2, losses_georeg, all_depth_loss, all_normal_loss) = loss_aux
  ...
  all_normal_loss = jax.lax.pmean(all_normal_loss, axis_name='batch')
  # host_callback.call(lambda x : np.save(f"distance_mean.npy", x), distance_mean)
  # host_callback.call(lambda x : np.save(f"area_split.npy", x), area_split)
  ...
  stats = TrainStats(
      ...
      all_normal_loss = all_normal_loss
  )
    ...
    if jax.host_id() == 0:
      ...
      if step % config.print_every == 0:
        ...
        print(f'{step:{precision}d}' + f'/{config.max_steps:d}: ' +
              ...
              f', depthloss={stats.all_depth_loss[-1]:0.4f}' + 
              f', normal_loss={stats.all_normal_loss[-1]:0.4f}')
